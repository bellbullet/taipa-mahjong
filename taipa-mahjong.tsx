...
const GOLD   = "#d4a017";
const POLL_MS = 1500;
const ROOM_TTL_MS = 60 * 60 * 1000;   // 1時間で期限切れ
const MATCH_OPTIONS = [1, 3, 5];      // 先取本数
...
// ── 得点計算 ─ 連番+2, 同数+1, 早上がり+1（マッチ最初の1人）, リーチ+1, ドラ+1
...
// ── ゲーム初期化 ─ 既存のスコア・先取本数・マッチ最初の上がり情報を引き継ぐ
function initAI(carry = {}) {
  ...
  return {
    ...
    pDiscard: [], aDiscard: [],
    firstWinner,                  // マッチ全体での最初の上がり者
    score, matchWins, target,
    msg: "あなたのターンです",
  };
}
...
async function cleanupOldRooms() {
  try {
    const r = await window.storage.list("room:", true);
    if (!r?.keys) return;
    const now = Date.now();
...
export default function App() {
  ...
  const [target, setTarget] = useState(3);        // 先取本数
...
function Rules({ onBack }) {
  ...
  return (
    <div style={containerStyle}>
      ...
      <div style={sec}>
        <div style={h2}>👥 オンライン対戦の注意</div>
        <p style={li}>• 同じClaudeアーティファクトを開いた相手とのみ対戦可能</p>
        <p style={li}>• ターン同期は約1.5秒間隔のポーリング</p>
        <p style={li}>• 内輪のカジュアルプレイ向け（対戦データは共有ストレージ経由）</p>
      </div>
    ...
    (async () => {
      try {
        ...
        while (exists && tries < 8) {
          c = genCode();
          try { await window.storage.get(`room:${c}`, true); } catch { exists = false; }
          tries++;
        }
        if (cancelRef.current) return;
        await window.storage.set(`room:${c}`, JSON.stringify({
        ...
        const poll = async () => {
          if (cancelRef.current) return;
          try {
            const r = await window.storage.get(`room:${c}`, true);
            const data = JSON.parse(r.value);
            if (data.guestId) {
              const initState = initMP({ target });
              await window.storage.set(`room:${c}`, JSON.stringify({
                ...data, state: initState, ver: 1,
              }), true);
        ...
        if (!cancelRef.current) setStatus(`エラー: ${e.message || "部屋作成失敗"}`);
      }
    })();
    ...
    <div style={containerStyle}>
      ...
      {code ? (
        <>
          ...
          {copied && (
            <div style={{ color: "#5ef57c", fontSize: 11, marginBottom: 8, letterSpacing: 2 }}>
              ✓ コピーしました
            </div>
          )}
        ...
        <div style={{ background: PANEL, border: `1px solid ${GOLD}33`, borderRadius: 12, padding: 40, marginBottom: 16 }}>
...
function MPJoin({ onReady, onBack }) {
  ...
  const join = async () => {
    ...
    try {
      ...
      await window.storage.set(`room:${c}`, JSON.stringify({ ...data, guestId: myId }), true);
  ...
  return (
    <div style={containerStyle}>
      ...
      <div style={{ display: "flex", gap: 10, width: "100%", maxWidth: 320 }}>
        <button
          onClick={join} disabled={joining || code.length !== 4}
          style={{
    ...
    const t = setTimeout(() => {
      upd(g => {
        ...
        if (chkWin(h6)) {
          ...
          const newMatch = { ...g.matchWins, p: g.matchWins.p + 1 };
          return {
            ...g, wall, pHand: h6,
    ...
    const t = setTimeout(() => {
      upd(g => {
        ...
        if (chkWin(h6)) {
          ...
          const newMatch = { ...g.matchWins, a: g.matchWins.a + 1 };
          return {
            ...g, wall, aHand: h6,
...
function MPGame({ room, onExit }) {
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      if (cancel) return;
      try {
        const r = await window.storage.get(`room:${room.code}`, true);
        const data = JSON.parse(r.value);
        missCount = 0;
        if (data.ver > verRef.current && !writingRef.current) {
  ...
  const write = useCallback(async (newState) => {
    ...
    try {
      const r = await window.storage.get(`room:${room.code}`, true);
      const data = JSON.parse(r.value);
      await window.storage.set(`room:${room.code}`, JSON.stringify({
        ...data, state: newState, ver: nv,
      }), true);
    } catch (e) {
      setErr("通信エラー: " + (e.message || ""));
    }
  ...
  const onNext = () => {
    if (!mpState) return;
    if (matchOver) {
      write(initMP({ target: mpState.target }));     // 完全リセット
    } else {
      write(initMP({
  ...
  const exit = async () => {
    try { await window.storage.delete(`room:${room.code}`, true); } catch {}
    onExit();
  };
  ...
  if (err) {
    return (
      <div style={containerStyle}>
        <div style={{ marginTop: 60, color: "#f5705e", fontSize: 16 }}>⚠ {err}</div>
        <button onClick={exit} style={{
          marginTop: 20, padding: "12px 32px", background: GOLD, color: "#091509",
          border: "none", borderRadius: 9, fontWeight: 900, cursor: "pointer",
          letterSpacing: 3, fontFamily: "inherit",
        }}>戻る</button>
      </div>
    );
...
function GameBoard({ g, sel, setSel, doDiscard, declareRiichi, onNext, onExit, labels, mpInfo, matchOver, matchWinner }) {
  ...
  // 待ち牌（聴牌時=rq、もしくはリーチ中の表示用）
  const waits = (g.phase === "rq" || (g.pRiichi && g.phase !== "end"))
    ? tenpai(g.pHand) : [];
  ...
  return (
    <div style={containerStyle}>
      ...
      {/* 相手手牌 */}
      <div style={{
        background: PANEL, border: `1px solid #f5705e33`,
      ...
      {/* 自分手牌 */}
      <div style={{
        background: PANEL, border: `1px solid #5eb8f533`,
        borderRadius: 12, padding: "12px 16px",
        width: "100%", maxWidth: 460, marginBottom: 10,
      }}>
